const frontendUrl = "https://localhost:5500/files";
...
// to hide unknown oerflow in files page when it is displayed first time
...
async function getUserData(url) {
  ...
  <div class="table-cell action-cell">
    ...
    var files = document.querySelectorAll(".files-table-row");
    ...
    const actionButtonFunction = (searching = false, searchIndex) => {
      ...
      for (let index = 0; index < shareButtons.length; index++) {
        ...
        shareButtons[index].addEventListener("click", () => {
          ...
          shareCopyBtn.addEventListener("click", (event) => {
            sharePopup = false;
            ...
            try {
              copyToClipboard(shareLink.innerHTML);
      ...
      for (let index = 0; index < renameButtons.length; index++) {
        renameButtons[index].addEventListener("click", () => {
          ...
          renameBtn.addEventListener("click", () => {
            ...
            files[index].childNodes[1].innerHTML = renameInput.value;
            if (searching) {
              data[searchIndex[index]].originalfilename = renameInput.value;
            } else {
              data[index].originalfilename = renameInput.value;
            }

            toastText.innerHTML = "File Renamed"; //green
            toastFunction("cornflowerblue");
            //renamed
          });
        });
      ...
      for (let index = 0; index < deleteButtons.length; index++) {
        deleteButtons[index].addEventListener("click", () => {
          ...
          files[index].remove();
          toastText.innerHTML = "Deleted"; //green
          toastFunction("cornflowerblue");
        });
      }
      ...
      // Remove files more then 20 end

      // Adding more content in scroll
      let scroll = 0;
      let showNext = 30;
      document
        ...
        .addEventListener("scroll", function (event) {
          ...
          console.log(scroll);
          showNext += 1;
          if (showNext <= files.length) {
            for (let index = 20; index < showNext + 9; index++) {
              try {
                if (files[index].classList.contains("display-none")) {
                  files[index].classList.remove("display-none");
                }
      ...
      // checkForNewDiv();
      // Adding more content in scroll end
    };
...
const uploadFiles = (() => {
  ...
  return (files, options = defaultOptions) => {
    ...
    // const allFiles = ;
    uploadFilesArray.push(...files);
    console.log(uploadFilesArray);
    function wait() {
      if (nextFile < uploadFilesArray.length) {
        if (stopWaiting) {
          ...
          console.log(uploadFilesArray[nextFile]);
          const file = uploadFilesArray[nextFile];
...
fileInput.addEventListener("change", (e) => {
  console.log(e.currentTarget.files[0].name.length);
  if (e.currentTarget.files.length === 1) {
    if (e.currentTarget.files[0].name.length >= 80) {
      toastText.innerHTML = "File Name Too Long";
    ...
    toastText.innerHTML = "Cannot add more then 10 files";
    return toastFunction();
  } else {
    for (let index = 0; index < e.currentTarget.files.length; index++) {
      if (e.currentTarget.files[index].name.length >= 80) {
        toastText.innerHTML =
          " File name Too Long\n" + e.currentTarget.files[index].name;
        return toastFunction();
      }
